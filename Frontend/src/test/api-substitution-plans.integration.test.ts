...
import { Route } from "../routes/api/substitution/plans";
...
describe("substitution plans API route timeout handling", () => {
  it("aborts slow upstream requests and returns 504 Gateway Timeout", async () => {
    ...
    try {
      ...
      const responsePromise = handler({
        request: new Request(
          "https://dashboard.local/api/substitution/plans?screen=hall",
        ),
      });
  ...
  it("returns fallback 503 Service Unavailable when upstream is unreachable", async () => {
    const handler = getSubstitutionPlansGetHandler();
