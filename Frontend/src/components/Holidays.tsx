  ...
  useEffect(() => {
    const loadHolidays = () => {
      ...
      try {
        ...
        setError(
          "Fehler beim Laden der Feriendaten. Bitte kontaktiere Cédric.",
        );
      } finally {
        setLoading(false);
      }
    ...
    <div className="bg-white rounded-lg shadow-md p-4 mb-4 w-full">
        ...
        {holidays.slice(0, 4).map((holiday, index) => (
          ...
          >
            <div className="flex items-start">
              <Calendar
                size={24}
                className={`mr-3 mt-1 ${getEventIconColor(holiday.type, index === 0)}`}
              />
              <div>
                ...
                <p
                  className={`text-sm font-medium ${index === 0 ? "text-[#8C7356]" : "text-gray-500"} mt-1`}
                >
      ...
      <div className="mt-4 text-xs text-gray-500 text-center">
        <p>
          ...
          bereitgestellt von der Senatsverwaltung für Bildung, Jugend und
          Familie Berlin.
        </p>
      </div>
    </div>
