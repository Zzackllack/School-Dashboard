...
const useAutoScroll = (basePauseDuration = 500, baseScrollSpeed = 40) => {
  ...
  useEffect(() => {
    ...
    const scrollToBottom = () => {
      ...
      console.log(
        `[AutoScroll] Scrolling down with duration: ${scrollDuration}ms`,
      );
      ...
      const scrollStep = (timestamp: number) => {
        ...
        if (progress < 1) {
          ...
          console.log(
            `[AutoScroll] Reached bottom, pausing for ${basePauseDuration}ms`,
          );
          setTimeout(() => {
    ...
    const scrollToTop = () => {
      ...
      console.log(
        `[AutoScroll] Scrolling up with duration: ${scrollDuration}ms`,
      );
      ...
      const scrollStep = (timestamp: number) => {
        ...
        if (progress < 1) {
          ...
          console.log(
            `[AutoScroll] Reached top, pausing for ${basePauseDuration}ms`,
          );
          setTimeout(() => {
    ...
    const initialTimer = setTimeout(() => {
      if (isActive && documentHeight > window.innerHeight) {
        console.log(
          `[AutoScroll] Starting initial scroll with document height: ${documentHeight}px`,
        );
        scrollToBottom();
      } else if (isActive) {
        console.log(
          `[AutoScroll] Content fits in viewport (${documentHeight}px), not scrolling`,
        );
      }
    }, 2000);
