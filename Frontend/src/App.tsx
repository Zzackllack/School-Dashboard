...
const App = () => {
  ...
  useEffect(() => {
    const updateHeight = () => {
      if (planRef.current) {
        setPlanHeight(planRef.current.offsetHeight);
      }
    };
    ...
    const currentRef = planRef.current;

    // Set up a resize observer to track changes in height
    ...
    <div className="flex flex-col min-h-screen w-full bg-gradient-to-b from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
      ...
      <main className="flex flex-col px-4 py-6">
        <div className="flex flex-col lg:flex-row w-full gap-5">
          <div ref={planRef} className="lg:w-3/4 col-span-2">
            <SubstitutionPlanDisplay />
          </div>
          <div
            className="lg:w-1/4 flex flex-col gap-5 overflow-y-auto"
