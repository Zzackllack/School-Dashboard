...
public class SubstitutionPlanPersistenceService {
	...
	public SubstitutionPlanDocument store(TimeTable table, SubstitutionPlan plan, String rawHtml) {
		...
		Optional<SubstitutionPlanDocument> existing = repository.findByPlanUuidAndDetailUrl(planUuid, detailUrl)
		...
		SubstitutionPlanDocument document = new SubstitutionPlanDocument(planUuid, table.getGroupName(),
				plan != null ? plan.getDate() : table.getDate(), resolveFileName(detailUrl), detailUrl, rawHtml,
		...
		try {
			...
			return repository.findByPlanUuidAndDetailUrl(planUuid, detailUrl)
					.or(() -> repository.findByDetailUrl(detailUrl)).orElseThrow(() -> ex);
		}
	}
