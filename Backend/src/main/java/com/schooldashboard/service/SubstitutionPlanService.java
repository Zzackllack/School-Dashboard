...
public class SubstitutionPlanService {
  ...
  public SubstitutionPlanService(
      DSBService dsbService,
      SubstitutionPlanParserService parserService,
  ...
  /**
   * Fetches the latest substitution plans from the DSB service and parses them This method is
   * scheduled to run every 5 minutes and evicts the cache
   */
    ...
    try {
      ...
      for (TimeTable table : timeTables) {
        if (table.getDetail() != null && !table.getDetail().isEmpty()) {
          logger.info(
              "[SubstitutionPlanService] Found timetable: UUID={} , Group={} , Detail URL={}",
              table.getUUID(),
              table.getGroupName(),
              table.getDetail());
          ...
          logger.info(
              "[SubstitutionPlanService] Skipping timetable with empty detail URL: {}",
              table.getTitle());
        }
      }
      logger.info(
          "[SubstitutionPlanService] Found {} valid timetables in {} distinct groups",
          validTables,
      ...
      for (UUID uuid : timeTablesByUuid.keySet()) {
        ...
        logger.info(
            "[SubstitutionPlanService] Processing group: {} with {} tables (UUID: {})",
            groupName,
            tables.size(),
            uuid);
        ...
        for (TimeTable table : tables) {
          try {
            logger.info(
                "[SubstitutionPlanService]   - Processing detail URL: {}", table.getDetail());
            ParsedPlanDocument parsedDocument =
                parserService.parsePlanDocumentFromUrl(table.getDetail());
            ...
            logger.info(
                "[SubstitutionPlanService]     Date: {} , Entries: {} , News items: {}",
                plan.getDate(),
                plan.getEntries().size(),
                plan.getNews().getNewsItems().size());

            // For the first plan, initialize the combined plan
            if (combinedPlan == null) {
              logger.info(
                  "[SubstitutionPlanService]     Initializing combined plan with first page");
              combinedPlan = plan;
              ...
              logger.info(
                  "[SubstitutionPlanService]     Added {} entries. Total now: {}",
                  plan.getEntries().size(),
                  combinedPlan.getEntries().size());
              ...
              logger.info(
                  "[SubstitutionPlanService]     Added {} unique news items. Total now: {}",
                  newNewsItems,
                  combinedPlan.getNews().getNewsItems().size());
            }
          } catch (Exception e) {
            logger.error(
                "[SubstitutionPlanService] ERROR parsing plan from URL {}: {}",
                table.getDetail(),
                e.getMessage());
            e.printStackTrace();
          }
        ...
        if (combinedPlan != null) {
          ...
          logger.error(
              "[SubstitutionPlanService] WARNING: Failed to create combined plan for UUID {}",
              uuid);
        }
      }

      logger.info(
          "[SubstitutionPlanService] Created {} combined plans, now sorting...",
          combinedPlans.size());

      // Sort plans by priority (heute first, then morgen, then others)
      // Then by date if available
      combinedPlans.sort(
          Comparator.comparing(SubstitutionPlan::getSortPriority)
              .thenComparing(
                  (p1, p2) -> {
      ...
      for (int i = 0; i < combinedPlans.size(); i++) {
        ...
        logger.info(
            "[SubstitutionPlanService]   {}. Priority={} , Date={} , Entries={}",
            i + 1,
            plan.getSortPriority(),
            plan.getDate(),
            plan.getEntries().size());
      }
      ...
      logger.info(
          "[SubstitutionPlanService] Updated substitution plans at {} , found {} plans in {}ms",
          new java.util.Date(),
          combinedPlans.size(),
          duration);
      ...
      logger.error(
          "[SubstitutionPlanService] CRITICAL ERROR updating substitution plans: {}",
          e.getMessage());
      e.printStackTrace();
    }
    logger.info("===============================================================");
