 ...
 *
 * <p>Attribution: The request/response extraction logic in this class is based on (and adapted
 * from) Sematre's open-source project "DSBmobile-API": https://github.com/Sematre/DSBmobile-API
 *
...
public class DSBMobile implements Serializable {
  ...
  public ArrayList<TimeTable> getTimeTables() {
    ...
    if (resultCode != 0)
      throw new RuntimeException(
          "Server returned result code is "
              + resultCode
              + ": "
              + mainObject.get("ResultStatusInfo").getAsString());

    JsonObject contentObject =
        findJsonObjectByTitle(mainObject.get("ResultMenuItems").getAsJsonArray(), "Inhalte");
    Objects.requireNonNull(contentObject, "Server response doesn't contain content!");

    JsonObject tableObject =
        findJsonObjectByTitle(contentObject.get("Childs").getAsJsonArray(), "Pläne");
    Objects.requireNonNull(tableObject, "Server response doesn't contain a table!");

    ArrayList<TimeTable> tables = new ArrayList<>();
    for (JsonElement jElement :
        tableObject.get("Root").getAsJsonObject().get("Childs").getAsJsonArray()) {
  ...
  public ArrayList<News> getNews() {
    ...
    JsonObject tableObject =
        findJsonObjectByTitle(contentObject.get("Childs").getAsJsonArray(), "News");
  ...
  public JsonObject pullData() {
    try {
      HttpsURLConnection connection =
          (HttpsURLConnection)
              new URL("https://www.dsbmobile.de/JsonHandler.ashx/GetData").openConnection();
      ...
      return gson.fromJson(
          Base64.decompress(
              gson.fromJson(builder.toString(), JsonObject.class).get("d").getAsString()),
          JsonObject.class);
    } catch (IOException e) {
