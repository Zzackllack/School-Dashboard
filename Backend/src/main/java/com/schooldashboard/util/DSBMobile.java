...
public class DSBMobile implements Serializable {
	...
	public ArrayList<TimeTable> getTimeTables() {
		...
		if (resultCode != 0)
			throw new RuntimeException("Server returned result code is " + resultCode + ": "
	...
	public JsonObject pullData() {
		...
		try {
			connection = openConnection(getEndpointUrl());
			...
			try (java.io.OutputStream out = connection.getOutputStream()) {
				out.write(packageArgs().getBytes("UTF-8"));
			}
			...
			JsonObject response = gson.fromJson(builder.toString(), JsonObject.class);
			return gson.fromJson(Base64.decompress(response.get("d").getAsString()), JsonObject.class);
		} catch (IOException e) {
	...
	private JsonObject findJsonObjectByTitle(JsonArray sourceArray, String title) {
		for (JsonElement jElement : sourceArray) {
			...
			if (objectTitle.equalsIgnoreCase(title))
				return jObject;
		}
